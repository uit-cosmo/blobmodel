...
def test_periodicity_four_passes():
    """
    Tests that periodic blobs are summed in with periodic_y=True when a blob has moved vertically through all
    the discretization domain.
    """
    ...
    # At time t=8 the blob will have gone 4 times around the vertical dimension of the domain and be at the same
    # vertical position as it started
    x = np.arange(0, 10, 1)
    y = np.arange(0, 10, 1)
    mesh_x, mesh_y = np.meshgrid(x, y)
    blob_values = blob.discretize_blob(x=mesh_x, y=mesh_y, t=8, periodic_y=True, Ly=10)

    # The blob is located at the position of maximum intensity
    max_indexes = np.where(blob_values == blob_values.max())
    assert max_indexes[0] == 5
    assert max_indexes[1] == 8


def test_periodicity_four_passes_with_pos_x():
    """
    Tests that periodic blobs are summed in with periodic_y=True when a blob has moved vertically through all
    the discretization domain.
    """
    ...
    y = np.arange(0, 10, 1)
    mesh_x, mesh_y = np.meshgrid(x, y)
    blob_values = blob.discretize_blob(x=mesh_x, y=mesh_y, t=4, periodic_y=True, Ly=10)

    # The blob is located at the position of maximum intensity
    max_indexes = np.where(blob_values == blob_values.max())
    assert max_indexes[0] == 5
    assert max_indexes[1] == 9
