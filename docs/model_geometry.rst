...
By setting the ``periodic_y`` argument of the ``Model`` class to ``True``, blobs that propagate out of the domain in the ``y`` direction enter at the opposit end. 
...
.. note::
   ...
   he code will give a warning if the blob width is less than ``0.1 * Ly``.
